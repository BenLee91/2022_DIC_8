...
# 초기 Setting
global X_data_study, Y_global, feature_selected, clf
form_class=uic.loadUiType('v3-UI_System.ui')[0]  # form_class변수에 ui호출
...
class MyWindow(QMainWindow, form_class):  # MyWindow Class로 생성 후, QMainWindow, UI 상속받기
    def __init__(self):
        ...
        self.L4_graph_features.addWidget(self.browser2)
        ...
        self.L4_btn_Predict.clicked.connect(self.L4_btn_Predictq)  # Layer4에서 Predict 진행

    def L0_btn_Excelfileq(self): #Template 폴더 선택/다운
        root=tkinter.Tk()
        root.withdraw()
        path=filedialog.askdirectory(initialdir="/", title="저장할 폴더위치를 선택해 주세요")#parent=root, initialdir="./", title="폴더를 선택해 주세요")
    ...
    def L2_btn_OKq(self):
        # Data 학습
        clf = RandomForestRegressor(random_state=1)  # RandomForest 사용
    ...
    def L3_btn_Analysisq(self):
        ...
        # Table 채우기 - Data간소화
        Selected_X_index_range= len(X_data_study)    # shape 구하기
        Selected_X_columns_range= len(X_data_study.columns)  # shape 구하기
        self.L4_tbl_Data.setRowCount(Selected_X_index_range)  # 최종table row 갯수 정하기
        self.L4_tbl_Data.setColumnCount(Selected_X_columns_range)  # 최종인자 table column 갯수 정하기
        self.L4_tbl_Data.setHorizontalHeaderLabels(X_data_study.columns) # 최종table column 값 채우기
        for row_index, row in enumerate(X_data_study.index):
            for col_index, column in enumerate(X_data_study.columns):
                value = X_data_study.loc[row_index][col_index]
                item = QTableWidgetItem(str(value))
                self.L4_tbl_Data.setItem(row_index, col_index, item)

        # Data ML 학습
        global clf
        self.X_train, self.X_test, self.y_train, self.y_test=train_test_split(X_data_study, Y_global, test_size=0.2, random_state=11) # 간소화된 X, Y data 분류하기
        clf = RandomForestRegressor(random_state=0)  # RandomForest 사용
        clf.fit(self.X_train, self.y_train)   # 학습
        testscore=round(clf.score(self.X_test,self.y_test),3) *100  # 학습 점수
        testscore_label=f'({testscore} %)'  #학습 점수
        self.L4_lbl_TestScore.setText(testscore_label) # 학습 점수

        # 학습 모델링 저장
        studied_data=pd.concat([X_data_study,Y_global], axis=1)
        studied_data.to_csv('file111.csv')
        saved_model=pickle.dumps(clf)
        joblib.dump(clf,'file11.pkl')

        # Table 채우기 - 추론Input영역
        Selected_X_columns_range = len(X_data_study.columns)  # shape 구하기
        self.L4_tbl_Input.setRowCount(Selected_X_columns_range)  # 최종table row 갯수 정하기
        self.L4_tbl_Input.setColumnCount(1)  # 최종인자 table column 갯수 정하기
        self.L4_tbl_Input.setHorizontalHeaderLabels(["Feature INPUT"])  # 최종table column 값 채우기

    def L4_btn_Predictq(self):
        # 추론
        predict_data=[]  # 예측Input 수집
        for i in range(len(X_data_study.columns)):  # 예측Input 수집
            # input값이 범주형이면, 어떻게 수치형으로 바꾸지?
            feature_input=int(self.L4_tbl_Input.item(i, 0).text()) # (i,0) cell 의 item 가져오기 (str type임)
            predict_data.append(feature_input)
            # if feature_input is None:  # 빈칸 입력 시 대처방법
            #     print("없음")
                # msg = QMessageBox.information(self, "(0,0) 내용")
        predict_data=pd.DataFrame(predict_data).T
        y_test_predict = clf.predict(predict_data)   # 예측Input 추론
        # 예상 그래프 표현
        # fig = px.bar(x, y_test_predict, title="Predict")
