...
class MyWindow(QMainWindow, form_class):  # MyWindow Class로 생성 후, QMainWindow, UI 상속받기
    ...
    def L1_btn_preprocessingq(self):
        ...
        self.Y_data = rawdata.iloc[:,31:35]    # Y인자 선정
        self.X_train, self.X_test, self.y_train[i], self.y_test=train_test_split(self.X_data, self.Y_data,test_size=0.2, random_state=11) # X, Y data 분류하기
        ...
        # Table 채우기 - Y인자
        Y_columns_range= len(self.Y_data.columns)  # shape 구하기
        self.L2_tbl_Ydata.setRowCount(X_index_range)  # Y인자 table row 갯수 정하기
        self.L2_tbl_Ydata.setColumnCount(Y_columns_range)  # X인자 table column 갯수 정하기
        # self.L2_tbl_Ydata.setHorizontalHeaderLabels('Noise') # Y인자 table column 값 채우기
        for row_index, row in enumerate(self.Y_data.index):
            for col_index, column in enumerate(self.Y_data.columns):
                value = self.Y_data.loc[row_index][col_index]
                item = QTableWidgetItem(str(value))
                self.L2_tbl_Ydata.setItem(row_index, col_index, item)
